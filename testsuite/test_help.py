...
class FSEnum(SimpleTestCase):
    ...
    def test_help(self):
        ...
        self.assertRegexpMatches(
            help_shell_output, '\[shell\.sh\] %s[\s\S]+usage:[\s\S]+\[shell\.php\] %s[\s\S]+usage:[\s\S]+' % (sh_descr, php_descr))
        ...
        self.assertRegexpMatches(
            help_shell_sh_output, 'usage:[\s\S]+%s[\s\S]+positional arguments:[\s\S]+optional arguments:[\s\S]+stored arguments:[\s\S]+' % (sh_descr))
