...
class SimpleTestCase(unittest.TestCase):
    ...
    def _setenv(cls):
        cls.basedir = os.path.join(
            conf['env_base_writable_web_dir'], randstr(4))
    ...
    def _run_cmd(cls, cmd, weevely=True):
        if weevely:
            command = '%s %s %s %s' % (
                conf['cmd'], conf['url'], conf['pwd'], cmd)
        else:
            command = cmd
    ...
    def _env_newfile(cls, relpath, content='1'):
        ...
        abspath = os.path.join(cls.basedir, relpath)
        cmd = Template(conf['env_cp_command']).safe_substitute(
            frompath=frompath, topath=abspath)
        cls._run_cmd(cmd)
    ...
    def _env_chmod(cls, relpath, mode='0744'):
        ...
        cmd = Template(conf['env_chmod_command']).safe_substitute(
            path=abspath, mode=mode)
    ...
    def _env_cp(cls, absfrompath, reltopath):
        ...
        cmd = Template(conf['env_cp_command']).safe_substitute(
            frompath=absfrompath, topath=abstopath)
...
class ProxyTestCase(RcTestCase):
    ...
    def _setenv(cls):
        ...
        start_new_thread(
            start_dummy_tcp_server, ('localhost', cls.dummyserverport))
