...
class Edit(SimpleTestCase):
    ...
    def test_edit(self):
        ...
        self.assertTrue(
            self._res(""":file.edit %s -editor "%s" """ % (temp_filename, self.editor)))
        self.assertTrue(
            self._res(""":file.edit %s -editor "%s" """ % (temp_filename, self.editor)))
        ...
        self.assertRegexpMatches(self._warn(
            """:file.edit /tmp/non/existant -editor "%s" """ % (self.editor)), modules.file.edit.WARN_UPLOAD_FAILED)
        self.assertRegexpMatches(self._warn(
            """:file.edit /etc/protocols -editor "%s" """ % (self.editor)), modules.file.edit.WARN_UPLOAD_FAILED)

    def test_edit_on_unwritable_folder(self):

        self.assertTrue(
            self._res(""":file.edit %s -editor "%s" """ % (self.writable_file, self.editor)))
    ...
    def test_keeping_timestamp(self):
        ...
        self.assertEqual(
            self._res(""":file.touch -epoch 1 %s """ % (self.writable_file2)), 1)
        self.assertTrue(self._res(
            """:file.edit %s -editor "%s" -keep-ts""" % (self.writable_file2, self.editor)))
        self.assertEqual(
            self._res(":file.read %s" % self.writable_file2), '11')
        self.assertEqual(
            self._res(""":file.check %s time_epoch""" % (self.writable_file2,)), 1)

        # Edit unkeeping timestamp
        self.assertTrue(self._res(
            """:file.edit %s -editor "%s" """ % (self.writable_file2, self.editor)))
        self.assertNotEqual(
            self._res(""":file.check %s time_epoch""" % (self.writable_file2,)), 1)

        # Edit new file keeping timestamp (?)
        self.assertTrue(self._res(
            """:file.edit %s -editor "%s" -keep-ts""" % (self.writable_file3, self.editor)))
        self.assertEqual(self._res(":file.read %s" % self.writable_file3), '1')
        self.assertNotEqual(
            self._res(""":file.check %s time_epoch""" % (self.writable_file3,)), 1)
