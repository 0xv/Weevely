...
class Terminal(Helper):
    ...
    def __init__(self, modhandler):
        ...
        self._load_rcfile(
            self.modhandler.sessions.get_session()['global']['rcfile'])

        # Register methods to dump files at exit
        atexit.register(
            readline.write_history_file, os.path.join(cfgfilepath, historyfilepath))
        atexit.register(
            modhandler.sessions.dump_all_sessions, modhandler.modules)
    ...
    def loop(self):
        ...
        while self.modhandler.interpreter:
            ...
            prompt = '{user}@{host}:{path} {prompt} '.format(
                ...
                host=hostname,
                path=getattr(
                    self.modhandler.load('shell.php').stored_args_namespace, 'path'),
            ...
            if input_cmd and (input_cmd[0] == ':' or input_cmd[:2] in ('ls', 'cd')):
                # This is a module call, pre-split to simulate argv list to
                # pass to argparse
                try:
                    cmd = shlex.split(input_cmd)
                except ValueError:
                    self._tprint(
                        '[terminal] [!] Error: command parse fail%s' % os.linesep)
                    continue
    ...
    def run_cmd_line(self, command):
        ...
        try:
            ...
            if command[0] == help_string:
                if len(command) == 2:
                    ...
                    if command[1] in self.modhandler.modules_classes.keys():
                        ...
                        self._tprint(self._format_helps(
                            [m for m in self.modhandler.modules_classes.keys() if command[1] in m], summary_type=1))
                else:
            ...
            # Set call if ":set module" or ":set module param value"
                ...
                self._tprint(
                    self.modhandler.load(command[1]).format_stored_args() + os.linesep)

            # Load call
            elif command[0] == load_string and len(command) == 2:
                # Recursively call run_cmd_line() and return to avoid to
                # reprint last output
            ...
            # Handle session management
                ...
                if len(command) >= 3 and command[1].startswith('http'):
                    self.modhandler.sessions.load_session(
                ...
                if command[0][0] == module_trigger:
                    ...
                    cmd = command[1:]
                # Raw command call. Command is re-joined to be considered as
                # single command
                else:
                    # If interpreter is not set yet, try to probe automatically
                    # best one
            ...
            self._tprint('[%s] [!] Error: %s%s' %
                         (e.module, e.error, os.linesep))
    ...
    def __guess_best_interpreter(self):
        ...
        # Run an empty command on shell.sh, to trigger first probe and load
        # correct vector
    ...
    def __cwd_handler(self, cmd=None):
        ...
        if cmd == None or len(cmd) == 1:
            cwd_new = Vector(
                self.modhandler,  'first_cwd', 'system.info', 'cwd').execute()
        elif len(cmd) == 2:
            cwd_new = Vector(self.modhandler,  'getcwd', 'shell.php',
                             'chdir("$path") && print(getcwd());').execute({'path': cmd[1]})
            if not cwd_new:
                self._tprint("[!] Folder '%s' change failed, no such file or directory or permission denied%s" % (
                    cmd[1], os.linesep))
                return

        if cwd_new:
            self.modhandler.load(
                'shell.php').stored_args_namespace['path'] = cwd_new
    ...
    def __env_init(self):
        ...
        username = Vector(
            self.modhandler, "whoami", 'system.info', "whoami").execute()
        hostname = Vector(
            self.modhandler, "hostname", 'system.info', "hostname").execute()
    ...
    def _init_completion(self):
        ...
        self.matching_words = [':%s' % m for m in self.modhandler.modules_classes.keys(
        )] + [help_string, load_string, set_string, session_string]
        ...
        try:
            ...
            readline.read_history_file(
                os.path.join(cfgfilepath, historyfilepath))
    ...
    def _complete(self, text, state):
        ...
        try:
            ...
            results = [
                c + ' ' for c in self.matching_words if c.startswith(cmd)] + [None]
            if len(results) == 2:
                if results[state]:
