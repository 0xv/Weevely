...
class Sessions():
        ...
        for sect in default_session:
            if not sect in session_dict:
                raise ModuleException(
                    "session", "%s '%s'" % (WARN_BROKEN_SESS, sect))
    ...
    def _load_session_by_url(self, url, password):
        ...
        sessions_available = glob.glob(
            os.path.join(cfgfilepath, '*', '*%s' % cfgext))

        for session in sessions_available:
            session_opts = self._load_session_by_file(
                session, just_return=True)
    ...
    def _guess_first_usable_session_name(self, hostfolder, bd_fixedname):
        ...
        while True:
            bd_filename = bd_fixedname + \
                (str(bd_num) if bd_num else '') + cfgext
    ...
    def _init_new_session(self, url, password, session_name=None):
        ...
        if not session_name:
            ...
            bd_fixedname = os.path.splitext(
                os.path.basename(urlparse.urlsplit(url).path))[0]

            session_name = self._guess_first_usable_session_name(
                hostfolder, bd_fixedname)
