...
class ModuleBase:
    ...
    def _init_session_args(self):
        ...
        session_args = self.modhandler.sessions.get_session().get(
            self.name, {})
    ...
    def store_args(self, submitted_args):
        ...
        # Else, store them
            ...
            self.stored_args_namespace = self.argparser.parse_args(
                submitted_args, self.stored_args_namespace)
    ...
    def format_stored_args(self):
        ...
        for index, argument in enumerate(action.dest for action in self.argparser._actions if action.dest != 'help'):
            value = self.stored_args_namespace[argument] if (
                argument in self.stored_args_namespace and self.stored_args_namespace[argument] != None) else ''
