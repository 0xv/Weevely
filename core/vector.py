...
class Vector:
    ...
    def execute(self, format_list={}, return_out_res=False):
        ...
        if not isinstance(format_list, DictType):
            raise Exception("[!][%s] Error, format vector type is not dict: '%s'" % (
                self.name, format_list))
        ...
        for payload in self.payloads:
            ...
            list_of_key_formats_in_payload = [
                s for s in format_template_list if '$%s' % s in payload]
            ...
            if dict_of_formats_in_payload:
                formatted_list.append(
                    Template(payload).safe_substitute(**dict_of_formats_in_payload))
            else:
