...
class Scan(Module):
    ...
    def _set_args(self):
        self.argparser.add_argument(
            'addr', help='Single IP, multiple: IP1,IP2,.., networks IP/MASK or firstIP-lastIP, interfaces (ethN)')
        self.argparser.add_argument(
            'port', help='Single post, multiple: PORT1,PORT2,.. or firstPORT-lastPORT')
        self.argparser.add_argument(
            '-unknown', help='Scan also unknown ports', action='store_true')
        self.argparser.add_argument(
            '-ppr', help=SUPPRESS, default=10, type=int)
    ...
    def _prepare(self):
        ...
        try:
            ...
            raise ProbeException(self.name,  '\'%s\' %s' %
                                 (services_path, WARN_NO_SUCH_FILE))
    ...
    def _probe(self):
        ...
        while self.args['reqs']:
            ...
            result = self.support_vectors.get('scan').execute(
                {'post_field': randstr(), 'data': b64encode('%s' % reqstringarray[:-1])})
            if result != '.':
...
class RequestList(dict):
    ...
    def __set_port_ranges(self, given_range):
        ...
        if given_range.count('-') == 1:
            try:
                splitted_ports = [int(strport) for strport in given_range.split(
                    '-') if (int(strport) > 0 and int(strport) <= 65535)]
            except ValueError:
                return None
            else:
        ...
        if start_port and end_port:
            self.port_list += [
                p for p in range(start_port, end_port + 1) if p in self.nmap_ports]
        else:
            raise ModuleException(
                'net.scan', 'Error parsing port numbers \'%s\'' % given_range)
