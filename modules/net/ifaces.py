...
class Ifaces(Module):
    ...
    def _set_vectors(self):
        ...
        self.support_vectors.add_vector(
            "ifconfig", 'shell.sh', "$ifconfig_path")

    def _probe(self):

        self._result = {}

        enum_pathlist = str([x + 'ifconfig' for x in ['/sbin/', '/bin/',
                                                      '/usr/bin/', '/usr/sbin/', '/usr/local/bin/', '/usr/local/sbin/']])

        ifconfig_pathlist = self.support_vectors.get(
            'enum').execute({'pathlist': enum_pathlist})

        for path in ifconfig_pathlist:
            if ifconfig_pathlist[path] != ['', '', '', '']:
                result = self.support_vectors.get(
                    'ifconfig').execute({'ifconfig_path': path})

                if result:
                    ifaces = re.findall(
                        r'^(\S+).*?inet addr:(\S+).*?Mask:(\S+)', result, re.S | re.M)
                    ...
                    raise ProbeException(
                        self.name, '\'%s\' %s' % (path, WARN_NO_OUTPUT))
