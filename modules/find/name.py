...
if($h) { @closedir($h); } }""")
        self.vectors.add_vector(
            "find", 'shell.sh', "find $rpath $recursion $mode \"$string\" 2>/dev/null")
    ...
    def _set_args(self):
        ...
        self.argparser.add_argument(
            '-equal', help='Match if name is exactly equal (default: match if contains)', action='store_true', default=False)
        self.argparser.add_argument(
            '-case', help='Case sensitive match (default: insenstive)', action='store_true', default=False)
