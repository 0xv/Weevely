...
class Suidsgid(Module):
    ...
    def _set_vectors(self):
        self.support_vectors.add_vector(
            "find", 'shell.sh', "find $rpath $perm 2>/dev/null")

    def _set_args(self):
        self.argparser.add_argument(
            '-rpath', help='Remote starting path', default='/')
        self.argparser.add_argument(
            '-suid', help='Find only suid', action='store_true')
        self.argparser.add_argument(
            '-sgid', help='Find only sgid', action='store_true')
