...
class Mount(Upload2web):
    ...
    def _set_args(self):
        ...
        self.argparser.add_argument(
            '-remote-mount', help='Mount remote folder, default: \'.\'', default='.')
        self.argparser.add_argument(
            '-local-mount', help='Mount to local mountpoint, default: temporary folder')

        self.argparser.add_argument(
            '-rpath', help='Upload PHP agent as rpath', nargs='?')
        self.argparser.add_argument(
            '-startpath', help='Upload PHP agent in first writable subdirectory', metavar='STARTPATH', default='.')

        self.argparser.add_argument(
            '-just-mount', metavar='URL', help='Mount URL without install PHP agent')
        self.argparser.add_argument(
            '-just-install', action='store_true', help="Install remote PHP agent without mount")
        self.argparser.add_argument(
            '-umount-all', action='store_true', help='Umount all mounted HTTPfs filesystems')

        self.argparser.add_argument(
            '-httpfs-path', help='Specify HTTPfs binary path if not in system paths', default='httpfs')

        self.argparser.add_argument(
            '-force', action='store_true', help=SUPPRESS)
        self.argparser.add_argument(
            '-chunksize', type=int, default=1024, help=SUPPRESS)
        self.argparser.add_argument(
            '-vector', choices=self.vectors.keys(), help=SUPPRESS)

    def _set_vectors(self):
        Upload2web._set_vectors(self)

        self.support_vectors.add_vector(
            "exists", 'file.check', "$rpath exists".split(' '))
    ...
    def _probe(self):
        ...
        if not self.support_vectors.get('exists').execute({'rpath': self.args['remote_mount']}):
            raise ProbeException(
                self.name, '%s \'%s\'' % (WARN_HTTPFS_MOUNTPOINT, self.args['remote_mount']))

        self.args['remote_mount'] = self.support_vectors.get(
            'normalize').execute({'path': self.args['remote_mount']})
        ...
        if not self.args['just_install']:
            ...
            cmd = '%s mount %s %s %s' % (self.args['httpfs_path'], self.args[
                                         'url'], self.args['local_mount'], self.args['remote_mount'])
    ...
    def _stringify_result(self):
        ...
        if not self.args['umount_all'] and not self.args['just_mount'] and not self.args['just_install']:
            ...
            if urlparsed.hostname:
                remoteuri = '%s:%s' % (
                    urlparsed.hostname, self.args['remote_mount'])

            rpath = ' '
            if self.args['rpath']:
                rpath = ' \'%s\' ' % self.args['rpath']

            self._output = WARN_MOUNT_OK % (
                remoteuri, self.args['local_mount'], self.args['url'], rpath)
    ...
    def __umount_all(self):
        ...
        if status != 0 or not output:
            raise ProbeException(
                self.name, '%s: %s' % (WARN_FUSE_UMOUNT, output))
    ...
    def __check_httpfs(self):
        ...
        status, output = getstatusoutput(
            '%s --version' % self.args['httpfs_path'])
        if status != 0 or not output:
            raise ModuleException(
                self.name, '\'%s\' %s' % (self.args['httpfs_path'], WARN_ERR_RUN_HTTPFS))

    def __generate_httpfs(self):

        status, php_bd_content = getstatusoutput(
            '%s generate php' % (self.args['httpfs_path']))
        if status != 0 or not php_bd_content:
            raise ProbeException(self.name, '\'%s\' %s' %
                                 (self.args['httpfs_path'], WARN_ERR_GEN_PHP))
