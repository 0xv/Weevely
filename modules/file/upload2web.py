...
class WebEnv:
    ...
    def __init__(self, support_vectors, url):
        ...
        self.base_folder_url = urlunsplit(
    ...
    def folder_map(self, relative_path_folder='.'):
        ...
        absolute_path = self.support_vectors.get(
            'normalize').execute({'path': relative_path_folder})

        if not absolute_path:
            raise ProbeException(self.name, '\'%s\' %s' %
                                 (relative_path_folder, WARN_NOT_FOUND))

        if not absolute_path.startswith(self.base_folder_path.rstrip('/')):
            raise ProbeException(self.name, '\'%s\' not in \'%s\': %s' % (
                absolute_path, self.base_folder_path.rstrip('/'), WARN_NOT_WEBROOT_SUBFOLDER))
    ...
    def file_map(self, relative_path_file):
        ...
        absolute_path_folder, url_folder = self.folder_map(
            relative_path_folder)
...
class Upload2web(Upload):
    ...
    def _set_args(self):
        ...
        self.argparser.add_argument(
            'rpath', help='Optional, upload as rpath', nargs='?')

        self.argparser.add_argument(
            '-startpath', help='Upload in first writable subdirectory', metavar='STARTPATH', default='.')
    ...
    def _set_vectors(self):
        ...
        self.support_vectors.add_vector(
            'find_writable_dirs', 'find.perms', '-type d -writable $path'.split(' '))
        self.support_vectors.add_vector(
            'document_root', 'system.info', 'document_root')
        self.support_vectors.add_vector(
            'normalize', 'shell.php', 'print(realpath("$path"));')
        self.support_vectors.add_vector(
            'script_folder', 'shell.php', 'print(dirname(__FILE__));')
    ...
    def _prepare(self):
        ...
        if self.args['rpath']:
            ...
            self.args['rpath'], self.args[
                'url'] = webenv.file_map(self.args['rpath'])
        else:
