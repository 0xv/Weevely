...
class Edit(Module):
    ...
    def _set_vectors(self):
        self.support_vectors.add_vector(
            'download', 'file.download', ["$rpath", "$lpath"])
        self.support_vectors.add_vector(
            'upload', 'file.upload', ["$lpath", "$rpath", "-force"])
        self.support_vectors.add_vector('md5', 'file.check', ["$rpath", "md5"])
        self.support_vectors.add_vector(
            'exists', 'file.check', ["$rpath", "exists"])
        self.support_vectors.add_vector(
            'get_time', 'file.check', ["$rpath", "time_epoch"])
        self.support_vectors.add_vector(
            'set_time', 'file.touch', ["$rpath", "-epoch", "$epoch"])

    def _set_args(self):
        self.argparser.add_argument('rpath', help='Remote path')
        self.argparser.add_argument(
            '-editor', help='Choose editor. default: vim',  default='vim')
        self.argparser.add_argument(
            '-keep-ts', help='Keep original timestamp',  action='store_true')
    ...
    def _probe(self):
        ...
        if not self.support_vectors.get('upload').execute({'rpath': self.args['rpath'], 'lpath': lpath}):
            ...
            raise ProbeException(self.name, '%s \'%s\' %s' % (
                WARN_UPLOAD_FAILED, self.args['rpath'], recover_msg))

        if self.args['keep_ts'] and self.args.get('epoch', None):
            new_ts_output, new_ts = self.support_vectors.get(
                'set_time').execute(self.args, return_out_res=True)
