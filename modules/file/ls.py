...
class Ls(ModuleGuess):
    ...
    def _set_vectors(self):
        ...
        self.vectors.add_vector(name='ls_php', interpreter='shell.php', payloads=[
                                "$p=\"$rpath\"; if(@is_dir($p)) { $d=@opendir($p); $a=array(); if($d) { while(($f = @readdir($d))) { $a[]=$f; }; sort($a); print(join('\n', $a)); } } else { print($p); }"])
        self.vectors.add_vector(
            name='ls', interpreter='shell.sh', payloads=['ls "$rpath" $args'])

        self.support_vectors.add_vector(
            'exists_and_writ', 'shell.php',  "$f='$rpath'; if(file_exists($f) && ((is_dir($f) && is_readable($f)) || !is_dir($f))) print(1); else print(0);")

    def _set_args(self):
        self.argparser.add_argument('rpath', nargs='?', default='.')
        self.argparser.add_argument(
            'args', nargs='*', help='Optional system shell \'ls\' arguments, preceeded by --')
    ...
    def _stringify_result(self):
        if self._result:
            # Fill self._result with file list only with ls_php vector or clean
            # ls execution
            if self.current_vector.name == 'ls_php' or (self.current_vector.name == 'ls' and not self.formatted_args['args']):
                ...
                self._output = '\n'.join(
                    [f for f in self._result if f not in ('.', '..')])
            else:
