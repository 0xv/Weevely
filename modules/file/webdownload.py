...
class Webdownload(ModuleGuess):
    ...
    def _set_vectors(self):
        ...
        self.vectors.add_vector(name='putcontent', interpreter='shell.php', payloads=[
                                'file_put_contents("$rpath", file_get_contents("$url"));'])
        self.vectors.add_vector(
            name='wget', interpreter='shell.sh', payloads=['wget $url -O $rpath'])
        self.vectors.add_vector(
            name='curl', interpreter='shell.sh', payloads=['curl -o $rpath $url'])

        self.support_vectors.add_vector(
            name='check_download', interpreter='file.check', payloads=['$rpath', 'exists'])
    ...
    def _verify_vector_execution(self):
        ...
        self._result = self.support_vectors.get(
            'check_download').execute({'rpath': self.args['rpath']})
