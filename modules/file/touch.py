...
class Touch(ModuleGuess):
    ...
    def _set_vectors(self):
        ...
        self.vectors.add_vector(name='touch_php', interpreter='shell.php', payloads=[
                                "touch('$rpath', '$epoch_time');"])
        self.vectors.add_vector(name='touch', interpreter='shell.sh', payloads=[
                                'touch -d @$epoch_time "$rpath" '])

        self.support_vectors.add_vector(
            name="exists", interpreter='file.check', payloads=['$rpath', 'exists'])
        self.support_vectors.add_vector(
            name='get_epoch', interpreter='file.check', payloads=['$rpath', 'time_epoch'])
        self.support_vectors.add_vector(
            name='ls', interpreter='file.ls', payloads=['$rpath'])

    def _set_args(self):
        self.argparser.add_argument('rpath')
        self.argparser.add_argument(
            '-time', help='Use timestamp like \'2004-02-29 16:21:42\' or \'16:21\'')
        self.argparser.add_argument('-epoch', help='Use epoch timestamp')
        self.argparser.add_argument('-ref', help='Use other file\'s time')
        self.argparser.add_argument(
            '-oldest', action='store_true', help='Use time of the oldest file in same folder')
    ...
    def __get_epoch_ts(self, rpath):
        ...
        if self.support_vectors.get('exists').execute({'rpath': rpath}):
            ref_epoch = self.support_vectors.get(
                'get_epoch').execute({'rpath': rpath})

        if not ref_epoch:
            raise ProbeException(
                self.name, 'can\'t get timestamp from \'%s\'' % ref_epoch)
    ...
    def __get_oldest_ts(self, rpath, limit=5):
        ...
        file_ls_all = self.support_vectors.get(
            'ls').execute({'rpath': rfolder})
    ...
    def _prepare_vector(self):
        ...
        if self.args['oldest'] == True:
            ...
            self.formatted_args['epoch_time'] = self.__get_oldest_ts(
                self.args['rpath'])
            ...
            self.formatted_args['epoch_time'] = self.__get_epoch_ts(
                self.args['ref'])

        elif self.args['time']:
            self.formatted_args['epoch_time'] = int(
                time.mktime(dateutil.parser.parse(self.args['time'], yearfirst=True).timetuple()))

        else:
            raise ModuleException(
                self.name, 'Too few arguments, specify -time or -ref or -oldest')
    ...
    def _stringify_result(self):
        if self._result:
            self._output = 'Changed timestamp: %s' % datetime.datetime.fromtimestamp(
