...
class Systemfiles(Module):
    ...
    def _set_vectors(self):
        self.support_vectors.add_vector(
            'find', 'find.perms', ["$path", "$mode"])
        self.support_vectors.add_vector(
            'findfiles', 'find.perms', ["$path", "$mode", "-type", "f"])
        self.support_vectors.add_vector(
            'findnorecurs', 'find.perms', ["$path", "$mode", "-no-recursion"])
        self.support_vectors.add_vector('findfilesnorecurs', 'find.perms', [
                                        "$path", "$mode", "-no-recursion", "-type", "f"])
        self.support_vectors.add_vector('users', 'audit.etcpasswd', ["-real"])
        self.support_vectors.add_vector(
            'check', 'file.check', ["$path", "$attr"])

    def _set_args(self):

        self.audits = (
            'etc_readable', 'etc_writable', 'crons', 'homes', 'logs', 'binslibs', 'root')
        self.argparser.add_argument(
            'audit', default='all', choices=self.audits + ('all',), nargs='?')

    def __etc_writable(self):
        result = self.support_vectors.get('find').execute(
            {'path': '/etc/', 'mode': '-writable'})
    ...
    def __etc_readable(self):
        ...
        sensibles = ['shadow', 'ap-secrets', 'NetworkManager.*connections',
                     'mysql/debian.cnf', 'sa_key$', 'keys' '\.gpg', 'sudoers']
        sensibles_re = compile('.*%s.*' % '.*|.*'.join(sensibles))

        allresults = self.support_vectors.get('findfiles').execute(
            {'path': '/etc/', 'mode': '-readable'})
    ...
    def __crons(self):
        result = self.support_vectors.get('find').execute(
            {'path': '/var/spool/cron/', 'mode': '-writable'})
    ...
    def __homes(self):
        ...
        result = self.support_vectors.get('findnorecurs').execute(
            {'path': '/home/', 'mode': '-writable'})
        result += ['/root'] if self.support_vectors.get(
            'check').execute({'path': '/root', 'attr': 'write'}) else []
        ...
        result = self.support_vectors.get('findnorecurs').execute(
            {'path': '/home/', 'mode': '-executable'})
        result += ['/root'] if self.support_vectors.get(
            'check').execute({'path': '/root', 'attr': 'exec'}) else []
    ...
    def __logs(self):
        ...
        commons = ['lastlog', 'dpkg', 'Xorg', 'wtmp',
                   'pm', 'alternatives', 'udev', 'boot']
        commons_re = compile('.*%s.*' % '.*|.*'.join(commons))

        allresults = self.support_vectors.get('findfilesnorecurs').execute(
            {'path': '/var/log/', 'mode': '-readable'})
