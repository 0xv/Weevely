...
class Console(Module):
    ...
    def _set_vectors(self):
        ...
        self.support_vectors.add_vector('mysql', 'shell.php', [
        ...
        self.support_vectors.add_vector('mysql_fallback', 'shell.php', [
                                        """$r=mysql_query("$query");if($r){while($c=mysql_fetch_row($r)){foreach($c as $key=>$value){echo $value."\x00";}echo "\n";}};mysql_close();"""]),
        self.support_vectors.add_vector('pg', 'shell.php', [
                                        """if(pg_connect("host=$host user=$user password=$pass")){$r=pg_query("$query");if($r){while($c=pg_fetch_row($r)){foreach($c as $key=>$value){echo $value."\x00";}echo "\n";}};pg_close();}""" ]),
        self.support_vectors.add_vector('pg_fallback', 'shell.php', [
                                        """$r=pg_query("$query");if($r){while($c=pg_fetch_row($r)){foreach($c as $key=>$value){echo $value."\x00";} echo "\n";}};pg_close();"""])
    ...
    def _set_args(self):
        ...
        self.argparser.add_argument(
            '-dbms', help='DBMS', choices=['mysql', 'postgres'], default='mysql')
    ...
    def _prepare(self):
        ...
        self.args['vector'] = 'pg' if self.args[
            'dbms'] == 'postgres' else 'mysql'
    ...
    def _query(self, query):
        ...
        result = self.support_vectors.get(self.args['vector']).execute({'host': self.args[
            'host'], 'user': self.args['user'], 'pass': self.args['pass'], 'query': query})
    ...
    def _check_credentials(self):
        ...
        get_current_user = 'SELECT USER;' if self.args[
            'vector'] == 'postgres' else 'SELECT USER();'

        user = self.support_vectors.get(self.args['dbms']).execute({'host': self.args[
            'host'], 'user': self.args['user'], 'pass': self.args['pass'], 'query': get_current_user})
        ...
        if user:
            ...
            raise ProbeException(self.name, "%s of %s " %
                                 (WARN_CHECK_CRED, self.args['host']))
